...
foreach ($lineas as $linea) {
      ...
      $nada               = !empty($datos[3])  ? ($datos[3]) : '';
      $nada_dos           = !empty($datos[4])  ? ($datos[4]) : '';
      $direccion          = !empty($datos[5])  ? ($datos[5]) : '';
      $partido            = !empty($datos[6])  ? ($datos[6]) : '';
      $localidad          = !empty($datos[7])  ? ($datos[7]) : '';
      $pais               = !empty($datos[8])  ? ($datos[8]) : '';
      $tipo_entrada       = !empty($datos[9])  ? ($datos[9]) : '';
      $numero             = !empty($datos[10])  ? ($datos[10]) : '';
      $tarea              = !empty($datos[11])  ? ($datos[11]) : '';
      $latitud            = !empty($datos[12])  ? ($datos[12]) : '';
      $longitud           = !empty($datos[13])  ? ($datos[13]) : '';
      $altura             = !empty($datos[14])  ? ($datos[14]) : '';
      $foto               = !empty($datos[15])  ? ($datos[15]) : '';
      ...
      $empl  = $empleado;
      $insertar = "INSERT INTO atcreportesgpon( 
      fecha,
      hora,
      empleado,
      num_empleado,
...
header('Location: ../../ATC/Basico/gpon.php');
