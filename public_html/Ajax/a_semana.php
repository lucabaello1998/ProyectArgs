...
<?php 
include("../db.php"); 
$resu = mysqli_query($conn, "SELECT * FROM calendario WHERE (inicio BETWEEN '$fechamas' AND '$fechamas2') AND tarea = 'Auditoria de herramientas' AND estado <> 'Rechazado' ");  //////ARRAY DE ULTIMOS TECNICOS
...
while($row = mysqli_fetch_assoc($result_tasks))
{
  ...
  {
    ?>
      <span class="badge badge-pill badge-danger objeto m-1 p-1" style="cursor: pointer;" data-event='{ "title": "Auditoria de herramientas", "color": "#FFAA16", "tecnico": "<?php echo $row['tecnico']; ?>" }'><?php echo limitar_cadena($row['tecnico'],15) ?></span>
    <?php
  }
  else
  {
    ?>
      <span class="badge badge-pill badge-warning objeto m-1 p-1" style="cursor: pointer;" data-event='{ "title": "Auditoria de herramientas", "color": "#FFAA16", "tecnico": "<?php echo $row['tecnico']; ?>" }'><?php echo limitar_cadena($row['tecnico'],15) ?></span>
    <?php
  }
}
