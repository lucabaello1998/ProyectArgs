...
<!-- MESSAGES -->
			...
			<div class="row justify-content-center pr-2 pl-2 pt-2 pb-0">
				<div class="col-auto align-self-center p-0">
					<form action="./Guardar/save_fecha.php" method="POST">
						...
						<input type="hidden" name="link" value="../inicio_despacho.php">
	...
	<!-- TOTAL-->
		<?php
			...
			$garantia = mysqli_query($conn, "SELECT COUNT(tecnico) as 'g_justi_sin' FROM garantias WHERE fecharep LIKE '%$mes%' AND tecrep <> '' AND justificado = ''");
	...
	<div class="container-fluid pr-4 pl-4 pt-0 pb-0">
        ...
        <div class="row"> <!-- CALENDARIO -->
              ...
              <style>
				...
				<div class="row">
					<div class="col-lg-4 col-md-6 col-12 p-0"><!-- SUPERVISORES -->
						<div class="row justify-content-center rounded bg-white shadow m-1">
              <div class="col-12 p-1">
                <canvas id="supervisores" style="height:40vh; width:40vw"></canvas>
              </div>
            </div>
					</div>
					<div class="col-lg-8 col-md-6 col-12 p-0"><!-- PENALIZACIONES -->
						<div class="row justify-content-center rounded bg-white shadow m-1">
              <div class="col-12 p-1">
                <canvas id="penalizaciones" style="height:40vh; width:40vw"></canvas>
              </div>
            </div>
					</div>
				</div>
              ...
              <div class="col-12 p-1">
								<nav>
									<div class="nav nav-tabs justify-content-center" id="nav-tab" role="tablist">
										<a class="nav-item nav-link active" id="nav-herramientas-tab" data-toggle="tab" href="#nav-herramientas" role="tab" aria-controls="nav-herramientas" aria-selected="true">Herramientas</a>
                    <a class="nav-item nav-link" id="nav-vehiculo-tab" data-toggle="tab" href="#nav-vehiculo" role="tab" aria-controls="nav-vehiculo" aria-selected="false">Vehiculos</a>
										<a class="nav-item nav-link" id="nav-instalaicones-tab" data-toggle="tab" href="#nav-instalaicones" role="tab" aria-controls="nav-instalaicones" aria-selected="false">Instalaciones</a>
									</div>
								</nav>
								<div class="tab-content" id="nav-tabContent">
									<div class="tab-pane fade show active" id="nav-herramientas" role="tabpanel" aria-labelledby="nav-herramientas-tab">

										<div class="row justify-content-center"> <!-- HERRAMIENTAS -->
											<div class="col-auto p-1 mx-auto">
												<table class="table table-striped table-bordered table-md table-sm table-responsive" style="max-height: 35rem;">
													<thead class="thead-dark text-center">
														<tr>
															<tr>
																<th>Tecnico</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Cantidad de auditorias de herramientas en el mes">Auditorias</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Ultima auditoria">Fecha</th>
															</tr>
														</tr>
													</thead>
													<tbody>
														<?php
															$au1 = mysqli_query($conn, "SELECT * FROM produccion  WHERE fecha like '%$mes%' GROUP BY tecnico ORDER BY tecnico asc");  
															while($row = mysqli_fetch_assoc($au1))
															{ $tec = $row['tecnico'];
														?>
															<tr>
															<td><?php echo $tec; ?></td>
																<?php
																	$au2 = mysqli_query($conn, "SELECT COUNT(tecnico) as 'cant_aud', tecnico FROM auditoria WHERE fecha like '%$mes%' AND tecnico= '$tec'");  
																	while($roww = mysqli_fetch_assoc($au2))
																	{
																?>
																	<td align="center"><?php echo $roww['cant_aud']; ?></td>
																<?php } ?>
																<?php
																	$ultima_audi = '-';
																	$au3 = mysqli_query($conn, "SELECT * FROM auditoria WHERE fecha like '%$mes%' AND tecnico= '$tec' LIMIT 1");  
																	while($rowww = mysqli_fetch_assoc($au3))
																	{
																		$ultima_audi = Fecha7($rowww['fecha']);
																		$id_a = $rowww['id'];
																	}
																?>
																	<?php if($ult_a_v == '-') {?>
																		<td align="center"><?php echo $ultima_audi; ?></td>
																	<?php } else { ?>
																		<td align="center"><a href="./Ver/ver_auditoria.php?id=<?php echo $id_a;?>" style="text-decoration: none;" class="text-dark"><?php echo $ultima_audi; ?></a></td>
																	<?php } ?>
															</tr>        
														<?php } ?>
													</tbody>
									...
									<div class="tab-pane fade" id="nav-vehiculo" role="tabpanel" aria-labelledby="nav-vehiculo-tab">
										...
										<div class="row justify-content-center"> <!-- VEHICULOS -->
											<div class="col-auto p-1 mx-auto">
												<table class="table table-striped table-bordered table-md table-sm table-responsive" style="max-height: 35rem;">
													<thead class="thead-dark text-center">
														<tr>
															<tr>
																<th>Tecnico</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Cantidad de auditorias de vehiculos en el mes">Auditorias</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Ultimo vehiculo">Patente</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Ultima auditoria">Fecha</th>
															</tr>
														</tr>
													</thead>
													<tbody>
														<?php
															$av1 = mysqli_query($conn, "SELECT * FROM produccion  WHERE fecha like '%$mes%' GROUP BY tecnico ORDER BY tecnico asc");  
															while($row = mysqli_fetch_assoc($av1))
															{ $tec = $row['tecnico'];
														?>
														<tr>
															<td><?php echo $tec; ?></td>
															<?php
																$av2 = mysqli_query($conn, "SELECT COUNT(tecnico) as 'cant_av' FROM auditoria_vehiculo WHERE fecha like '%$mes%' AND tecnico= '$tec'");  
																while($roww = mysqli_fetch_assoc($av2))
																{
															?>
																<td align="center"><?php echo $roww['cant_av']; ?></td>
															<?php } ?>
															<?php
																$ult_a_patente = '-';
																$av3 = mysqli_query($conn, "SELECT * FROM auditoria_vehiculo WHERE fecha like '%$mes%' AND tecnico= '$tec' LIMIT 1");  
																while($rowwww = mysqli_fetch_assoc($av3))
																{
																	$ult_a_patente = strtoupper($rowwww['dv_dominio']);
																}
															?>
															<td align="center"><?php echo $ult_a_patente; ?></td>
															<?php
																$ult_a_v = '-';
																$av4 = mysqli_query($conn, "SELECT * FROM auditoria_vehiculo WHERE fecha like '%$mes%' AND tecnico= '$tec' LIMIT 1");  
																while($rowww = mysqli_fetch_assoc($av4))
																{
																	$ult_a_v = Fecha7($rowww['fecha']);
																	$id_a_v = $rowww['id'];
																}
															?>
															<?php if($ult_a_v == '-') {?>
																<td align="center"><?php echo $ult_a_v; ?></td>
															<?php } else { ?>
																<td align="center"><a href="./Ver/ver_auditoria_vehiculos.php?id=<?php echo $id_a_v;?>" style="text-decoration: none;" class="text-dark"><?php echo $ult_a_v; ?></a></td>
															<?php } ?>
														</tr>        
														<?php } ?>
													</tbody>
									...
									<div class="tab-pane fade" id="nav-instalaicones" role="tabpanel" aria-labelledby="nav-instalaicones-tab">

                    <div class="row justify-content-center"> <!-- INSTALACIONES -->
											<div class="col-auto p-1 mx-auto">
												<table class="table table-striped table-bordered table-md table-sm table-responsive" style="max-height: 35rem;">
													<thead class="thead-dark text-center">
														<tr>
															<tr>
																<th>Tecnico</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Cantidad de auditorias de instalaciones en el mes">Auditorias</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Ultimo OT auditada">OT</th>
																<th data-toggle="tooltip" data-placement="bottom" title="Ultima auditoria">Fecha</th>
															</tr>
														</tr>
													</thead>
													<tbody>
														<?php
															$ai1 = mysqli_query($conn, "SELECT * FROM produccion  WHERE fecha like '%$mes%' GROUP BY tecnico ORDER BY tecnico asc");  
															while($row = mysqli_fetch_assoc($ai1))
															{ $tec = $row['tecnico'];
														?>
														<tr>
															<td><?php echo $tec; ?></td>
															<?php
																$ai2 = mysqli_query($conn, "SELECT COUNT(tecnico) as 'cant_ai' FROM auditoria_instalaciones WHERE fecha like '%$mes%' AND tecnico= '$tec'");  
																while($roww = mysqli_fetch_assoc($ai2))
																{
															?>
																<td align="center"><?php echo $roww['cant_ai']; ?></td>
															<?php } ?>
															<?php
																$ult_a_inst = '-';
																$ai3 = mysqli_query($conn, "SELECT * FROM auditoria_instalaciones WHERE fecha like '%$mes%' AND tecnico= '$tec' LIMIT 1");  
																while($rowwww = mysqli_fetch_assoc($ai3))
																{
																	$ult_a_inst = $rowwww['ot'];
																}
															?>
															<td align="center"><?php echo $ult_a_inst; ?></td>
															<?php
																$ult_a_i = '-';
																$ai4 = mysqli_query($conn, "SELECT * FROM auditoria_instalaciones WHERE fecha like '%$mes%' AND tecnico= '$tec' LIMIT 1");  
																while($rowww = mysqli_fetch_assoc($ai4))
																{
																	$ult_a_i = Fecha7($rowww['fecha']);
																	$id_a_i = $rowww['id'];
																}
															?>
															<?php if($ult_a_i == '-') {?>
																<td align="center"><?php echo $ult_a_i; ?></td>
															<?php } else { ?>
																<td align="center"><a href="./Ver/ver_auditoria_instalaciones.php?id=<?php echo $id_a_i;?>" style="text-decoration: none;" class="text-dark"><?php echo $ult_a_i; ?></a></td>
															<?php } ?>
														</tr>        
														<?php } ?>
													</tbody>
	...
	<div class="modal" id="tareas_calendario_despacho"  tabindex="-1" role="dialog"> <!-- MODAL CALENDARIO -->
	...
	<script>
				...
				eventClick: function(info, event)
        {
          ...
          $('#tareas_calendario_despacho').modal();
        },
			});
			calendar.render();
		});
